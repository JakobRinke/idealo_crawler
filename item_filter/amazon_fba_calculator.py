...
try:
    ...
    from amz_headers import get_headera
import time
...
def get_amazon_fees_json(item):
    resp = requests.post(AMAZON_FEE_URL, json=get_post_data( item), headers=get_header())
    return resp.json()

def get_shipping_fees(item, idealo_price):
    try:
        fees = get_amazon_fees_json( item)
...
if __name__ == '__main__':
    print(get_shipping_fees('B08RX4QKXS', 214.2))